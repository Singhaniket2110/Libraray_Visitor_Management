...
class SupabaseDirect:
    ...
    @classmethod
    def _calculate_duration_minutes(cls, entry_datetime, exit_datetime):
        """Calculate duration in minutes, handling next day exits correctly"""
        if not entry_datetime or not exit_datetime:
            return 0
        
        # If exit is before entry, assume exit is next day
    ...
    def insert_teacher(cls, teacher_data):
        ...
        try:
            ...
            data = {
                ...
                'purpose': teacher_data.get('purpose', 'Meeting'),
            ...
            print(f"👨‍🏫 TEACHER INSERT: {data['name']} at {data['entry_time']} IST")
    ...
    def get_today_teachers(cls):
        ...
        try:
            ...
            today = ist_now.date().isoformat()
            url = f"{Config.SUPABASE_URL}/rest/v1/teachers"
            params = {'visit_date': f'eq.{today}', 'select': '*', 'order': 'id.desc'}
    ...
    def update_teacher_exit_by_id(cls, teacher_id):
        """Update teacher exit time by ID with INDIAN TIME"""
        try:
            url = f"{Config.SUPABASE_URL}/rest/v1/teachers"
    ...
    def insert_visitor(cls, visitor_data):
        ...
        try:
            ...
            print(f"🇮🇳 STUDENT INSERT: {data['name']} at {data['entry_time']} IST")
    ...
    def update_exit_by_id(cls, visitor_id):
        """Update exit time by ID with INDIAN TIME - FIXED for next day"""
        try:
            url = f"{Config.SUPABASE_URL}/rest/v1/visitors"
            ...
            print(f"🕐 STUDENT EXIT: ID {visitor_id} at {exit_time} IST")
